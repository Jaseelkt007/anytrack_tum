...
- **2026-05-01 (M5)**: Lovable backtest fires. Probed gpt-engineer's 40K accessible stargazers; only 1 of our 28 active watchers (jeresig) had pre-Nov-2023 starred. Augmented active watchlist with 3 high-follower individuals who DID pre-star (`scriptedalchemy` 2939 followers, `koala73` 2703, `abdelstark` 817). Now 31 active members. After backfill via the regular pipeline (no special backtest loader needed — pipeline auto-creates OWNS_REPO when star's owner has a github identity), running convergence with `--as-of 2023-11-01 --window 365 --min-members 2` fires Anton Osika as the **#2 convergence (score=4.63, N=4 watchers)**. Live `/api/alerts` for Anton lists 4 click-through GitHub URLs to the actual stargazer events. **The headline demo moment works.**
- **2026-05-01 (M4)**: Convergence detector shipped. Walks both `FOLLOWS_ON_GITHUB` (watcher→target) and `STARRED_REPO + OWNS_REPO` (watcher stars repo owned by target) in one unified UNION query. Aggregation in Python; scoring = `distinct_member_count + recency_bonus`. 6 unit tests on the synthetic fixture (5 watchers / 3 targets, 2 fire / 1 doesn't) pass. Persisted as ConvergenceEvent nodes via MERGE on stable id (`cv-{user}-{target}-{window_end_date}`) — idempotent. Live run: 333 events fire on the 365-day window, 365 events on the 1100-day window. Backend `/api/founders` and `/api/alerts` switched to read persisted events instead of computing live, so STARRED_REPO+OWNS_REPO convergences (like Anton) surface in the live API.
